...
# ═══════════════════════════════════════════════════════
# 5. GANTT & REAL FX INDICATORS (unchanged)
# ═══════════════════════════════════════════════════════
...
def render_floorplan_diagram(plan, span=6.0):
    """Improved 2D floor plan with walls, doors as gaps, movement arrows."""
    # separate corridor and stairs from other rooms
    corridor = None
    stairs = None
    other_rooms = []
    for room in plan:
    ...
    corridor_len = corridor["h"]   # length along x
    corridor_wid = corridor["w"]   # width (thickness)
    # We'll place corridor horizontally from x=0 to x=corridor_len
    # Rooms are placed along the corridor, alternating above/below, with small gaps for doors.
    fig = go.Figure()

    unit_len = t("m") if st.session_state.get("unit_system") != "imperial" else t("ft")
    span_m = span

    # Helper to draw a room rectangle with thick border (walls)
    ...
    # Place rooms alternately above/below corridor
    current_x = 1.5   # start position for first room
    side = 1  # 1 = above, -1 = below
    for room in other_rooms:
        rw = room["w"]  # width along x
        rd = room["h"]  # depth along y
        if current_x + rw > corridor_len:
            current_x = 1.5   # wrap to next row, we'll just stack above (not perfect but acceptable)
            side = -side
        # y position: offset from corridor edge
        if side == 1:
        ...
        # Door gap and swing symbol
        door_x = current_x + rw/2
        if side == 1:
            door_y = corridor_wid/2
            # small gap in corridor wall (not really drawn, we just place an arc)
            # draw door swing arc
            ...
            fig.add_shape(type="path",
                          ...
                          line=dict(color="yellow", width=2),
                          fillcolor="rgba(255,255,0,0.2)")
        # Movement arrow from corridor to room entrance
    ...
    # Stairs: placed at the end of corridor if present
    if stairs:
...
# ═══════════════════════════════════════════════════════
    ...
    if trigger_synthesis:
        with st.spinner("🧠 Sai Engine synthesizing 5 architectural variations..."):
            concepts = []
            for i in range(5):
                mut_plot = input_plot + random.randint(-400, 400)  # increased mutation
